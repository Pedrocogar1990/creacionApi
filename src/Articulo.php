...
class Articulo extends Conexion
{
    ...
    public static function read($cat = null, $count = 20)
    {
        ...
        if ($count <= 0) $count = 20;
        $q = ($cat != "BAZAR" && $cat != "ALIMENTACION") ? "select * from articulos order by nombre LIMIT $count" : "select * from articulos where categoria='$cat'";
        $stmt = parent::$conexion->prepare($q);
        try {
            $stmt->execute();
        } catch (PDOException $ex) {
        ...
        $datos['AUTOR'] = "Pedro Colomina";
        $datos['TOTAL'] = $stmt->rowCount();
        if ($stmt->rowCount() == 0) {
            $datos['ARTICULOS'] = "No se han encontrado artículos";
        }
